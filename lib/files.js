...
function validateFileSizes(relPaths, repoRoot) {
  ...
  for (const relPath of relPaths) {
    ...
    if (size > 1048576) {
      error.push(
        relPath + " (" + Math.round((size / 1048576) * 10) / 10 + "MB)",
      );
    } else if (size > 512000) {
...
function sortExtraFilePaths(files) {
  files.sort((a, b) => {
    const aIsEnv = isEnvFilePath(a);
    const bIsEnv = isEnvFilePath(b);
...
async function getEnvFiles(repoRoot) {
  const files = [];
  ...
  function walk(currentDir) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (entry.isFile() && isEnvFilePath(relPath)) {
          files.push(relPath);
        }
      } catch (err) {
  ...
  walk(repoRoot);
  return sortExtraFilePaths([...new Set(files)]);
}
...
async function getGitignoreFiles(repoRoot) {
  ...
  try {
    ...
    return { files: sortExtraFilePaths(uniqueFiles), warnings };
  } catch (err) {
