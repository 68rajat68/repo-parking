...
/** AES-256-GCM over binary plaintext; returns raw bytes [iv|ct|tag] (not base64). */
function encryptBufferWithMEK(plaintextBuffer, mek) {
  const iv = crypto.randomBytes(16);
  const cipher = crypto.createCipheriv("aes-256-gcm", mek, iv);
  const encrypted = Buffer.concat([
...
/** Inverse of encryptBufferWithMEK. */
function decryptBufferWithMEK(ciphertextBuffer, mek) {
  const iv = ciphertextBuffer.slice(0, 16);
  const authTag = ciphertextBuffer.slice(ciphertextBuffer.length - 16);
  const ciphertext = ciphertextBuffer.slice(16, ciphertextBuffer.length - 16);
  const decipher = crypto.createDecipheriv("aes-256-gcm", mek, iv);
  decipher.setAuthTag(authTag);
  try {
