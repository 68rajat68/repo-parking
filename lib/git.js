...
/**
 * Commits on localBranch that are not contained in remoteTrackingRef.
 * Uses two-dot range (not symmetric three-dot): remote..local.
 */
async function countCommitsAhead(git, remoteTrackingRef, localBranch) {
...
/**
 * Resolve which remote-tracking ref to compare against for localBranch.
 * Prefer branch.<name>.merge + remote (via @{upstream}); never assume "origin".
 */
async function resolveTrackingRefForBranch(git, branchName) {
  try {
    const upstream = (
      await git.raw([
        "rev-parse",
  ...
  let remotesResult;
  try {
    remotesResult = await git.raw(["remote"]);
  } catch (err) {
    return null;
  }
  ...
  for (const remote of remotes) {
    const candidate = remote + "/" + branchName;
    try {
      await git.raw(["rev-parse", "--verify", "-q", candidate]);
      return "refs/remotes/" + candidate.replace(/\\/g, "/");
    } catch (err) {
      continue;
    }
  }
...
async function getAllBranchesWithUnpushed(repoRoot) {
  ...
  for (const branchName of branchNames) {
    try {
      const trackingRef = await resolveTrackingRefForBranch(git, branchName);
      if (!trackingRef) {
        continue;
      }

      const count = await countCommitsAhead(git, trackingRef, branchName);
      if (count > 0) {
