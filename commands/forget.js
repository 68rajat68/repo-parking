...
const { ensureVaultExists, loadProject, deleteProject, loadMeta, saveMeta, pushVault } = require('../lib/vault');
...
async function forgetCommand(nameOrLetter) {
  ...
  console.log('');
  console.log('This does NOT delete your GitHub repo. Only removes the vault entry.');
  console.log('\x1b[33m⚠ If you have not unparked this project recently, you will lose access to its .env and extra files.\x1b[0m');
  ...
  const projectDataPath = path.join(vaultPath, 'projects', project.id + '.json');
  ...
  if (!success) {
    ...
    console.log('Could not reach vault. No changes were made. Try again when online.');
    return;
  }

  console.log('Forgotten. Letter [' + project.id + '] is retired and will never be reused.');
}
