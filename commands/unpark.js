...
async function unparkCommand(nameOrLetter) {
  ...
  if (keyPath) {
    ...
    if (!result.success) {
      console.log('\x1b[33m⚠ Could not auto-load SSH key: ' + result.error + '\x1b[0m');
  ...
  if (project.remote_push_url) {
    simpleGit(restoreRoot).remoteSetUrl(['--push', 'origin', project.remote_push_url]);
  }
  ...
  if (project.parked_branch) {
    try {
      ...
      if (!branches.all.includes(project.parked_branch)) {
        ...
        try {
          await simpleGit(restoreRoot).checkout(['-b', project.parked_branch, 'origin/' + project.parked_branch]);
        } catch (err) {
          console.log('\x1b[33m⚠ Could not checkout branch ' + project.parked_branch + '. Using default branch.\x1b[0m');
        }
      }
    } catch (err) {
  ...
  for (const file of decryptedData.extraFiles) {
    try {
      ...
      console.error('Failed to restore extra file ' + file.path + ': ' + err.message);
    }
  }
  ...
  if (project.setup_cmd) {
    ...
    if (runSetup === 'yes') {
      ...
      if (result.status !== 0) {
        console.log('\x1b[33m⚠ Setup command exited with non-zero status\x1b[0m');
      }
    } else {
  ...
  console.log('');
  console.log('Restored to ' + (overwriteConfirmed ? cloneTarget : path.join(process.cwd(), project.name)) + '. Ready to work.');
}
